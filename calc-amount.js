...
(function(){
  var ref$, times, minus, plus, div, toJS, transaction, calcFee, find, round5, roundNumber, contractData, calcCryptoGeneric, calcCryptoFromUsd, calcCryptoFromEur, calcFiat, calcUsd, calcEur, calcFeeProxy, calcFeeBeforeSend, changeAmountGeneric, changeAmountSend, changeAmountCalcFiat, changeAmountWithoutFee, changeAmount, changeAmountInvoice, out$ = typeof exports != 'undefined' && exports || this;
  ...
  out$.calcFeeBeforeSend = calcFeeBeforeSend = function(arg$, cb){
    var store, query, fast, token, to, data, network, amount, feeType, txType, account, swap, send, calcFeeFun, gasPrice;
    store = arg$.store, query = arg$.query, fast = arg$.fast;
    store = query.store, token = query.token, to = query.to, data = query.data, network = query.network, amount = query.amount, feeType = query.feeType, txType = query.txType, account = query.account, swap = query.swap;
    if (store == null) {
    ...
    gasPrice = (function(){
      ...
      case (send != null ? send.gasPriceType : void 8) !== 'custom':
        return times(send != null ? send.gasPriceCustomAmount : void 8, Math.pow(10, 9));
      default:
  ...
  changeAmountGeneric = function(field){
    return function(store, amountSend, fast, cb){
      var send, amountBuffer, wallet, token, wallets, feeToken, ref$, feeWallet, decimalsConfig, decimals, balance, maxAmount, resultAmountSend, feeType, txType, feeCustomAmount, usdRate, feeUsdRate, account, dataBuilder;
      ...
      store.current.send.homeFee = times(store.current.send.amountSend, div(store.current.send.homeFeePercent, 100));
      store.current.send.homeFeeUsd = times(store.current.send.homeFee, wallet.usdRate);
      ...
      return dataBuilder.formContractData(function(err){
        ...
        return calcFeeBeforeSend({
          ...
          if (result != null) {
            calcedFee = result.calcedFee, gasPrice = result.gasPrice, gasEstimate = result.gasEstimate;
  ...
  out$.changeAmountSend = changeAmountSend = function(store, amountSend, fast, cb){
    var send, wallet, token, wallets, feeToken, ref$, feeWallet, decimalsConfig, decimals, resultAmountSend, balance, maxAmount, feeType, txType, feeCustomAmount, usdRate, feeUsdRate, account, dataBuilder;
    send = store.current['send'];
    wallet = send.wallet;
    token = send.coin.token;
    wallets = store.current.account.wallets;
    ...
    return dataBuilder.formContractData(function(err){
      ...
      return calcFeeBeforeSend({
        ...
        var calcedFee, gasPrice, gasEstimate, txFee, txFeeIn, ref$, minusFee, resultAmountSend;
        if (err != null) {
          send.error = err;
        }
        if (err != null) {
        ...
        txFeeIn = wallet != null ? (ref$ = wallet.network) != null ? ref$.txFeeIn : void 8 : void 8;
        minusFee = (function(){
          var ref$;
          switch (false) {
  ...
  out$.changeAmountCalcFiat = changeAmountCalcFiat = function(store, amountSend, fast, cb){
    var send, wallet, token, wallets, feeToken, ref$, feeWallet, decimalsConfig, balance, decimals, resultAmountSend, feeType, txType, feeCustomAmount, usdRate, feeUsdRate, account, dataBuilder;
    send = store.current['send'];
    wallet = send.wallet;
    token = send.coin.token;
    wallets = store.current.account.wallets;
  ...
  out$.changeAmountWithoutFee = changeAmountWithoutFee = function(store, amountSend, fast, cb){
    var send, wallet, token, wallets, feeToken, ref$, feeWallet, decimalsConfig, decimals, balance, maxAmount, resultAmountSend, feeType, txType, feeCustomAmount, usdRate, feeUsdRate, account, txFee;
    send = store.current['send'];
    wallet = send.wallet;
    token = send.coin.token;
    wallets = store.current.account.wallets;
